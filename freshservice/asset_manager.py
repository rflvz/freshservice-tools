...
import logging
import time
from freshservice.config import DEFAULT_COLUMNS
...
class AssetManager(FreshServiceAPI):
    ...
    def _process_single_asset(self, asset_id, options):
        ...
        try:
            ...
            # Crear diccionario base con display_id, name y asset_type
            result = {
            ...
            # Obtener y añadir el tipo de asset siempre
            asset_type = self._get_asset_type(asset_data)
            result['asset_type'] = asset_type
            logger.debug(f"Added asset type: {asset_type}")
    ...
    def export_data(self, data, output_file=None, verbose=True):
        ...
        # Reordenar columnas según el orden definido en config
        existing_columns = [col for col in DEFAULT_COLUMNS if col in df.columns]
        remaining_columns = [col for col in df.columns if col not in DEFAULT_COLUMNS]
